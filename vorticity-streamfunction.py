...
def solvePoisson_Jacobi(p, f, deltax):
    ...
    while True:
        ...
        p[1:-1, 1:-1] = 0.25 * (
                    deltax * deltax * f[1:-1, 1:-1] + (pn[1:-1, 2:] + pn[1:-1, 0:-2] + pn[2:, 1:-1] + pn[0:-2, 1:-1]))
...
def solvePoisson_SOR(p, f, deltax):
    ...
    while True:
        ...
        for i in range(1, p.shape[0] - 1):
            for j in range(1, p.shape[1] - 1):
                p[i, j] = (1 - beta) * pn[i, j] + 0.25 * beta * (
                            deltax * deltax * f[i, j] + pn[i, j + 1] + p[i, j - 1] + pn[i + 1, j] + p[i - 1, j])
...
f[50,50] = 1/deltax/deltax
f[25,25] = -2/deltax/deltax
f[75,75] = -2/deltax/deltax

#Now we test the three different approaches
#Only the Jacobi method is parallized
p1=solvePoisson_Jacobi(p,f,deltax)
myplot3d(X,Y,p1)
#p2=solvePoisson_GaussSeidel(p,f,deltax)
#myplot3d(X,Y,p2)
#p3 =solvePoisson_SOR(p,f,deltax)
#myplot3d(X,Y,p3)
...
for tstep in range(maxstep):
    ...
    B = nu * deltat / deltax / deltax * (
                omc[2:, 1:-1] + omc[1:-1, 2:] - 4 * omc[1:-1, 1:-1] + omc[0:-2, 1:-1] + omc[1:-1, 0:-2])
...
ax2.streamplot(X[1:-1, 1:-1], Y[1:-1, 1:-1], u1.T, u2.T, color=uabs.T, linewidth=2,
               cmap=plt.cm.autumn)  # streamline plot
plt.show()